...
class SimpleMysql:
    ...
    def connect(self):
        ...
        try:
            self.conn = MySQLdb.connect(db=self.conf['db'], host=self.conf['host'],port=self.conf['port'], user=self.conf['user'],passwd=self.conf['passwd'],charset=self.conf['charset'])
...
if __name__ == '__main__':
    b = SimpleMysql(user='homestead', passwd='secret', db='webspider', host='192.168.10.10', charset="utf8", use_unicode=True)
    s = b.getAll(table='free_proxy_list',
        where = ("last_checked > %s AND checked_status = %s ", ["2016-09-19", "0"])
    )
    print len(s)
    print type(s)
