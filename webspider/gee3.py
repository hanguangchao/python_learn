...
##    driver = webdriver.Firefox(executable_path="/Users/han/bin/firefoxdriver")
...
#     鼠标点击元素并按住不放
    ...
    for track in track_list:
        track_string = track_string + "{%d,%d}," % (track, y - 592)
#         xoffset=track+22:这里的移动位置的值是相对于滑动圆球左上角的相对值，而轨迹变量里的是圆球的中心点，所以要加上圆球长度的一半。
#         yoffset=y-445:这里也是一样的。不过要注意的是不同的浏览器渲染出来的结果是不一样的，要保证最终的计算后的值是22，也就是圆球高度的一半
        ActionChains(driver).move_to_element_with_offset(to_element=element, xoffset=track+22, yoffset=y-592).perform()
#         间隔时间也通过随机函数来获得
