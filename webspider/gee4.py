...
#     这里的文件路径是webdriver的文件路径
    driver = webdriver.Chrome(executable_path=r"/Users/han/bin/chromedriver")

#     打开网页
    driver.get("http://www.gsxt.gov.cn/index.html")
    driver.find_element_by_id("keyword").clear()
    driver.find_element_by_id("keyword").send_keys(u"好租科技发展有限公司")
    time.sleep(0.8)
...
#     获得滑动圆球的高度 458.0
    y=location['y']
    offset_y = int(y - 22) 
    print "圆球高度" + str(y) + " \t y偏移量" + str(offset_y) 
#     鼠标点击元素并按住不放
    print "第一步,点击元素"
    ...
    for track in track_list:
        track_string = track_string + "{%d,%d}," % (track, y - offset_y )
#         xoffset=track+22:这里的移动位置的值是相对于滑动圆球左上角的相对值，而轨迹变量里的是圆球的中心点，所以要加上圆球长度的一半。
#         yoffset=y-445:这里也是一样的。不过要注意的是不同的浏览器渲染出来的结果是不一样的，要保证最终的计算后的值是22，也就是圆球高度的一半
        ActionChains(driver).move_to_element_with_offset(to_element=element, xoffset=track+22, yoffset= y - offset_y).perform()
#         间隔时间也通过随机函数来获得
        time.sleep(random.randint(10,50)/100)
    print track_string
#     xoffset=21，本质就是向后退一格。这里退了5格是因为圆球的位置和滑动条的左边缘有5格的距离
    ActionChains(driver).move_to_element_with_offset(to_element=element, xoffset=21, yoffset=y-offset_y).perform()
    time.sleep(0.1)
    ActionChains(driver).move_to_element_with_offset(to_element=element, xoffset=21, yoffset=y-offset_y).perform()
    time.sleep(0.1)
    ActionChains(driver).move_to_element_with_offset(to_element=element, xoffset=21, yoffset=y-offset_y).perform()
...
#     点击验证
    ...
    time.sleep(3)
    #url = 'http://www.gsxtgov.cn/corp-query-search-1.html'
    '''
    ...
    WebDriverWait(driver, 30).until(lambda the_driver: the_driver.find_element_by_xpath("//div[@class='search_result']").is_displayed())
    # 得到所有链接
    checkboxes = driver.find_elements_by_css_selector('a[class=\"search_list_item db\"]')
    print "所有链接======="
    for checkbox in checkboxes:
           print checkbox.get_attribute("href")
